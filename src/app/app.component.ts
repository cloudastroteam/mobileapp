import { Component, ViewChild } from '@angular/core';
import { Platform, Nav } from 'ionic-angular';
import { StatusBar } from '@ionic-native/status-bar';
import { SplashScreen } from '@ionic-native/splash-screen';

import { BucketsForYouPage } from '../pages/buckets-for-you/buckets-for-you';
import { MyBookingsPage } from '../pages/my-bookings/my-bookings';
import { BookMyBucketPage } from '../pages/book-my-bucket/book-my-bucket';
import {Globals} from '../app/globals'



@Component({
  templateUrl: 'app.html'
})
export class MyApp {
  @ViewChild(Nav) navCtrl: Nav;
    rootPage:any = BookMyBucketPage;
    
  constructor(platform: Platform, statusBar: StatusBar, splashScreen: SplashScreen, private globals: Globals) {
    
    platform.ready().then(() => {
      // Okay, so the platform is ready and our plugins are available.
      // Here you can do any higher level native things you might need.
      statusBar.styleDefault();
      splashScreen.hide();
    });
  }
  goToBucketsForYou(params){
    if (!params) params = {};
    this.navCtrl.setRoot(BucketsForYouPage);
  }goToMyBookings(params){
    if (!params) params = {};
    this.navCtrl.setRoot(MyBookingsPage);
  }
}
